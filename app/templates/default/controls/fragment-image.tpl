<div id="{$type}-image-fragment" class="row">
	...
	<div class="label">
		...
		{*<a class="red{if isset($fragmentImage) and $fragmentImage.{$type}.fragment neq ''}{else} hidden{/if}" data-js="remove-image-fragment" data-js-action="{$type}">Usuń</a>*}
	</div>
	<!-- <input id="{$type}-image-path" name="hidden[path]" type="hidden" value="{$sPath|default:''}" /> -->
	<div>
		<input id="form-{$type}-fragment" name="fragment[{$type}][fragment]" type="text" class="input" value="{$fragmentImage.{$type}.fragment|default:''}">
	</div>
</div>
<div id="{$type}-image-div" class="row{if isset($fragmentImage) and $fragmentImage.{$type}.fragment neq ''}{else} hidden{/if}">
	...
	<div>
		<figure class="image-fragment-preview">
			...
			<img id="{$type}-image-preview" src="{$site}{$fragmentImage.{$type}.fragment|default:''}" style="width: 100%; height: auto;" />
		</figure>
	</div>
</div>