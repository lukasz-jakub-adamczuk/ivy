    ...
    <form method="post" action="{$base}/{$ctrl}/index{if isset($tmpPath)}/{$tmpPath}{/if}">
        ...
        <div class="nav-bar with-borders bg-almost-white">
            ...
            <div class="related-actions dib ml">
                {if $tmpPath eq ''}
                    <span class="button disabled icon-folder"></span>
                {else}
                    <a href="{$base}/{$ctrl}/info-dir{if $tmpPath neq ''}/{$tmpPath}{/if}" class="button icon-folder">Otwórz folder</a>
                {/if}
                <a href="{$base}/{$ctrl}/info-dir-add{if $tmpPath neq ''}/{$tmpPath}{/if}" class="btn btn-secondary icon-folder"><sup class="icon-add"></sup>Nowy folder</a>
                <a href="{$base}/{$ctrl}/info-file-add{if $tmpPath neq ''}/{$tmpPath}{/if}" class="btn btn-secondary icon-file"><sup class="icon-add"></sup>Nowy plik</a>
            </div>
            
            <span id="toggle-layout">
                <span class="pill active icon-grid" data-layout="grid">Siatka</span>
                <span class="pill icon-list" data-layout="list">Lista</span>
            </span>
        </div>
        {/if}
        {if isset($dirs)}
        <div class="dir-content grid row">
        {foreach from=$dirs item=dir key=dk}
            {if $dir.name neq '..'}
            <input id="dir-{$dk}" name="dirs[]" type="checkbox" value="{$dir.name}" class="mass-checkbox">
            {/if}
            <div class="col-1">
        ...
        {foreach from=$files item=file key=fk}
            ...
            <div class="col-1">
                <span class="res {if $file.type eq 'image'}icon-image image-thumbnail{elseif $file.type eq 'audio'}icon-music{elseif $file.type eq 'video'}icon-film{else}icon-file{/if}">
                    ...
                    {image file=$file.path size=50x50}
                    {/if}
                </span>
