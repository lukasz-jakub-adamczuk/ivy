<div class="_row form-group">
								...
								<div>
									<select id="form-article" name="hidden[id_article]" class="form-control">
									...
									{if isset($aFields.id_article)}
										{foreach from=$aArticles item=a}
											{if $a.id_article eq $aFields.id_article}<input id="form-article-slug" name="hidden[article]" type="hidden" value="{$a.title|stripslashes}">
											<input id="form-article-abbr" name="hidden[abbr]" type="hidden" value="{$a.abbr|default:''}">{/if}
										{/foreach}
									{/if}
								</div>
								{/foreach}
