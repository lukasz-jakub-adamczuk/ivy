						...
						<div class="dir-content grid">
							...
							{if isset($aFiles)}
								{foreach from=$aFiles item=file key=fk}
									...
									<div>
										<span{* href="{$base}/{$ctrl}/index/{if $sPath}{$sPath},{/if}{$file}"*} class="res {if $file.type eq 'image'}icon-image image-thumbnail{elseif $file.type eq 'audio'}icon-music{elseif $file.type eq 'video'}icon-film{else}icon-file{/if}">
											{if $file.type eq 'image'}
											<img src="{$site}/image.php?img={if $sPath}{$sPath|replace:',':'/'}/{/if}{$file.name}&size=72x72">
											{/if}
										</span>
